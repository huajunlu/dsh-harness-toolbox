...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const zh = {
			...
			"usage.balanceLiveKey": "实时（API Key）",
			"usage.granted": "赠送",
			"usage.toppedUp": "充值",
			"usage.balanceUnsupported": "无余额接口",
			"usage.signedOut": "未在 DSH 内登录平台账号（浏览器登录与宿主授权是两回事）——当前余额来自 API Key 直连官方接口；登录后可查看平台钱包明细。",
			"usage.signedOutNoKey": "未在 DSH 内登录平台账号，且未能从凭据服务取得 DEEPSEEK_API_KEY，因此查不到余额。可在设置 →「账户」登录，或为 DEEPSEEK_API_KEY 配置凭据。",
		...
		const en = {
			...
			"usage.balanceLiveKey": "live (API key)",
			"usage.granted": "granted",
			"usage.toppedUp": "topped up",
			"usage.balanceUnsupported": "no balance API",
			"usage.signedOut": "Not signed in to the DeepSeek platform inside DSH (browser sign-in is separate from the host grant); this balance comes from the API key query. Sign in for platform wallet details.",
			"usage.signedOutNoKey": "Not signed in to the DeepSeek platform inside DSH, and no DEEPSEEK_API_KEY could be resolved, so the balance is unavailable. Sign in under Settings → Account, or configure the key.",
		...
		function UsageTab({ t }) {
			...
			const liveBalance = data.balanceSource === "live" || data.balanceSource === "live-api-key";
			const liveViaKey = data.balanceSource === "live-api-key";
				...
				(data.balances && data.balances.length > 0) || data.spend || data.balanceNote
					? h("div", { style: S.card },
						h("div", { style: S.row },
							...
							liveBalance
								? h("span", { style: S.badge("good") }, liveViaKey ? t("usage.balanceLiveKey") : t("usage.balanceLive"))
								: staleBalance
						...
						data.balances && data.balances.length > 0
							? h("div", { style: S.chips },
								data.balances.map((balance, index) =>
									h("span", { key: index, style: S.chip },
										balance.displayName + "　" + balance.totalBalance + " " + balance.currency +
										(balance.granted || balance.toppedUp
											? "（" + t("usage.granted") + " " + (balance.granted || "0") + " · " + t("usage.toppedUp") + " " + (balance.toppedUp || "0") + "）"
											: "")
									)
								)
							)
							: null,
						data.balanceNote === "signed-out-no-key"
							? h("div", { style: S.notice }, t("usage.signedOutNoKey"))
