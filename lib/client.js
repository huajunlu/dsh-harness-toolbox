		...
		function UpdaterTab({ t }) {
			...
			async function startUpgrade() {
				...
				try {
					...
					if (err.body && err.body.error) setNotice("⚠️ " + err.body.error);
					else setNotice("⚠️ " + err.message);
					if (String(err.message).includes("nonce")) await load();
				} finally {
					setBusy(false);
				}
			}
			...
			return h("div", { style: S.section },
				...
				h("div", { style: S.card },
					...
					h("div", { style: S.btnRow },
						h("button", { style: S.btn(false, busy || running), disabled: busy || running, onClick: () => doCheck() }, t("updater.check")),
						status.upgradeSupported && status.updateAvailable
							? (confirming
								? h(react.Fragment, null,
									...
									h("button", { style: S.btn(true, busy), disabled: busy, onClick: startUpgrade }, t("updater.confirm"))
								)
								: h("button", { style: S.btn(true, busy), disabled: busy, onClick: () => setConfirming(true) }, t("updater.upgrade"))
							)
							: null
					),
		...
		function UsageTab({ t }) {
			...
			if (!data.available) {
				return h("div", { style: S.card },
					...
					h("div", { style: S.hint }, t("usage." + (data.reason || "internal"))),
					h("div", { style: S.btnRow }, h("button", { style: S.btn(false, false), onClick: load }, t("common.refresh")))
				);
			}
			...
			return h("div", { style: S.section },
				...
				data.models && data.models.length > 0
					...
					: null,
				// 余额与消费
				(data.balances && data.balances.length > 0) || data.spend
					? h("div", { style: S.card },
						data.balances && data.balances.length > 0
							? h(react.Fragment, null,
								h("div", { style: S.cardTitle }, t("usage.balances")),
								h("div", { style: S.chips },
